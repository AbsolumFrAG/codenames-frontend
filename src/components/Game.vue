...
export default {
  ...
  methods: {
    ...
    cardCountDisplay(team) {
      if (this.board === []) return "0 / 0";
      return (
        this.board.filter((card) => card.type === team && card.flipped).length +
        " / " +
  ...
  computed: {
    player: function () {
      return this.room.players.find((player) => player.id == this.$socket.id);
    },
    board: function () {
    ...
    tabHidden: function () {
      return this.$vssWidth < 1280
        ? "fixed tab h-screen-fixed top-0 " +
            (this.$vssWidth < 640 ? "w-full" : "")
        : "";
    },
    tabPlayersLogic: function () {
      if (!this.playersOpen) {
        if (this.$vssWidth < 640) return "left-tab-full";
        if (this.$vssWidth < 1280) return "left-tab";
      } else {
        if (this.$vssWidth < 1280) return "left-0 z-60";
      }
      return "";
    },
    tabChatLogic: function () {
      if (!this.chatOpen) {
        if (this.$vssWidth < 640) return "right-tab-full";
        if (this.$vssWidth < 1280) return "right-tab";
      } else {
        if (this.$vssWidth < 1280) return "right-0 z-60";
      }
      return "";
    },
  },
