<template>
  <div
    class="bg-main-300 relative p-2 sm:p-0 sm:rounded-lg w-full mt-2"
    ...
    <div id="gameBoard">
      <sequential-entrance delay="100" tag="div">
        <Game-card
          ...
          @card-flipped="cardFlipped(card, ...arguments)"
        />
      </sequential-entrance>
    </div>
...
export default {
  ...
  computed: {
    isRevealed() {
      return (
        !this.loading && (this.gameover || (this.player.spymaster ?? false))
      );
    },
    canBeFlipped() {
      return (
        !this.gameover &&
        !this.player.spymaster &&
        this.player.team !== "spectator"
      );
    },
  },
  methods: {
