<template>
  <div :class="['rounded-lg w-full mb-4', bgColor]">
    ...
    >
      <span>{{ header }}</span>
      <Team-panel-button
        v-if="player.team != team"
        @on-click="joinTeam(team)"
        :team="team"
        label="Rejoindre"
        icon="fa-plus"
      />
    ...
    <div class="px-4 py-2 pl-2 text-white">
      ...
      <p
        ...
        class="text-center text-gray-100 pt-3 pb-2"
      >
        {{
          team == "spectator"
            ? "Plus de joueurs peuvent rejoindre à tout moment"
    ...
    <div class="flex flex-col mb-4 pb-2 px-2">
      <Team-panel-button
        v-if="team != 'spectator' && player.team == team && !player.spymaster"
        :team="team"
        @on-click="setSpymaster(true)"
        label="Devenir espion"
        icon="fa-bullhorn"
      />
      <Team-panel-button
        v-if="team != 'spectator' && player.team == team && player.spymaster"
        :team="team"
        @on-click="setSpymaster(false)"
        label="Devenir opérateur"
        icon="fa-user-secret"
      />
    </div>
  </div>
